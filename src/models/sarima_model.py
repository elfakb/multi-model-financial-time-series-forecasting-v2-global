...
def train(asset_name, period="5y"):
    ...
    print(f"[SARIMA] {asset_name} [{period}] eğitiliyor...")

    model = auto_arima(train_close, seasonal=True, m=5, stepwise=True, suppress_warnings=True, error_action="ignore")
    preds   = model.predict(n_periods=len(test_close))
    results = evaluate(test_close, preds, model_name="SARIMA")

    joblib.dump(model, f"{SAVE_DIR}/{period}/{asset_name}_sarima.pkl")

    with mlflow.start_run(run_name=f"{asset_name}_{period}_SARIMA"):
        ...
        mlflow.sklearn.log_model(model, "sarima_model", serialization_format="cloudpickle")

    return model, preds, results


def predict(asset_name, period="5y", n_days=30):
    model = joblib.load(f"{SAVE_DIR}/{period}/{asset_name}_sarima.pkl")
